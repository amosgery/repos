namespace MyFirstRazorApp.Model
{
    using Microsoft.Extensions.Configuration;
    using System.Data;
    using Microsoft.Data.SqlClient;
    using MyFirstRazorApp.Pages.Content;
    ...
    public class Helper
    {
        ...
        // if the user is already taken the method will return -1.
        {
            ...
            dr["Email"] = user.Email;
            dr["Phone"] = user.Phone;
            dr["Birthdate"] = user.Birthday.ToShortDateString();
            dr["Gender"] = user.Gender;
            dr["Occupation"] = user.Occupation;
        ...
        public int Update(User user, string table)
        {
            string SQL = $"UPDATE {table} " +
                ...
                $"Email = '{user.Email}', Phone = '{user.Phone}',  Admin = '{user.Admin.ToString()}', " +
                $"Birthdate = '{user.Birthday:dd-MM-yyyy}' " +
                $"WHERE Id = {user.ID}";
            int n = ExecuteNonQuery(SQL);
            return n;
        }
